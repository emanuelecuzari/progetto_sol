...
int print_cliente(argsClienti_t* cliente, const char* filename){
     ...
     * dati dei clienti
     * FORMATO
     * CLIENTE:id:tempo_permanenza:tempo_in_coda:t_servizio:numero_cambi:prodtti_acquistati;\n
    */
    ...
    fprintf(fp, "CLIENTE:%d:%.8f:%.8f:%.8f:%d:%d;\n", cliente->id, cliente->t_supermercato, t_incoda, cliente->t_servizio, cliente->cambi, cliente->num_prodotti);
